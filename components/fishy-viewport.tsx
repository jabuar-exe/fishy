...
export function FishyViewport(props:Props) {
  ...
  const runtime=useRef<{scene:T.Scene;renderer:T.WebGLRenderer;camera:T.PerspectiveCamera;orbit:OrbitControls;transform:TransformControls;content:T.Group;meshes:Map<string,T.Object3D>;outline:T.BoxHelper|null;drag:{id:string;base:number}|null;water:T.Mesh|null;waterFlow:WaterFlow|null;frame:()=>void}|null>(null);
  useEffect(()=>{
    const container=host.current;if(!container)return;
    let renderer:T.WebGLRenderer;
    try{renderer=new T.WebGLRenderer({antialias:true,alpha:true});}catch{setMessage("WebGL is unavailable. Try a browser with hardware acceleration.");return;}
    ...
    const frame=()=>{const {tank}=latest.current.scene;const box=latest.current.showcase?new T.Box3().setFromObject(content):new T.Box3(new T.Vector3(-tank.width/2,0,-tank.depth/2),new T.Vector3(tank.width/2,tank.height,tank.depth/2));if(box.isEmpty())return;const center=box.getCenter(new T.Vector3()),size=box.getSize(new T.Vector3());const distance=Math.max(size.y,size.x/camera.aspect,size.z)*2.5;camera.position.copy(center).add(new T.Vector3(.65,.4,1).multiplyScalar(distance));orbit.target.copy(center);orbit.update();};
    const state={scene,renderer,camera,orbit,transform,content,meshes:new Map<string,T.Object3D>(),outline:null as T.BoxHelper|null,drag:null as {id:string;base:number}|null,water:null as T.Mesh|null,waterFlow:null as WaterFlow|null,frame};runtime.current=state;
    ...
    const reduced=window.matchMedia("(prefers-reduced-motion: reduce)").matches;let animation=0,lastTime:number|null=null;
    const draw=(time:number)=>{animation=requestAnimationFrame(draw);orbit.update();constrain();state.outline?.update();if(state.water&&state.waterFlow&&!reduced){const elapsed=lastTime===null?0:(time-lastTime)/1000;lastTime=time;if(state.waterFlow.advance(elapsed)){const pos=state.water.geometry.getAttribute("position");for(let i=0;i<pos.count;i++)pos.setZ(i,state.waterFlow.heights[i]);state.water.geometry.computeVertexNormals();pos.needsUpdate=true;}}else lastTime=time;renderer.render(scene,camera);};animation=requestAnimationFrame(draw);setMessage("");
    return()=>{cancelAnimationFrame(animation);cancelAnimationFrame(initialFrame);observer.disconnect();renderer.domElement.removeEventListener("pointerdown",pointerDown);renderer.domElement.removeEventListener("pointerup",pointerUp);orbit.removeEventListener("change",constrain);orbit.dispose();transform.dispose();disposeObject(content);state.outline?.geometry.dispose();stageSetup.dispose();renderer.dispose();container.replaceChildren();runtime.current=null;};
  },[]);
  useEffect(()=>{
    ...
    disposeObject(w.content);w.content.clear();w.meshes.clear();w.water=null;w.waterFlow=null;
    if(props.showcase){setMessage("Loading original aquarium…");new GLTFLoader().load(`/showcase/${props.showcase}.glb`,g=>{if(abandoned){disposeObject(g.scene);return;}g.scene.traverse(o=>{if(o instanceof T.Mesh){o.castShadow=true;o.receiveShadow=true;}});w.content.add(g.scene);w.frame();setMessage("");},undefined,()=>{if(!abandoned){setMessage("Showcase could not load. Check connection and try another view.");latest.current.onError("The showcase asset failed to load.");}});return()=>{abandoned=true;};}
    const built=populateAquarium(w.content,props.scene,props.water);w.meshes=built.meshes;w.water=built.water;
    if(w.water){const geometry=w.water.geometry as T.PlaneGeometry,parameters=geometry.parameters;w.waterFlow=new WaterFlow(parameters.widthSegments+1,parameters.heightSegments+1,parameters.width,parameters.height);}
    const object=w.meshes.get(latest.current.selected??"");if(object){w.transform.attach(object);const helper=new T.BoxHelper(object,"#8bdace");w.outline=helper;w.scene.add(helper);}
    setMessage("");
    return()=>{abandoned=true;};
  },[props.scene,props.showcase,props.water]);
  useEffect(()=>{const w=runtime.current;if(!w||props.showcase)return;if(w.outline){w.scene.remove(w.outline);w.outline.geometry.dispose();(w.outline.material as T.Material).dispose();w.outline=null;}const object=w.meshes.get(props.selected??"");if(object){w.transform.attach(object);w.outline=new T.BoxHelper(object,"#8bdace");w.scene.add(w.outline);}else w.transform.detach();},[props.selected,props.showcase]);

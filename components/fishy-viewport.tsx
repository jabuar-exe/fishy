...
export function FishyViewport(props:Props) {
  ...
  const runtime=useRef<{scene:T.Scene;renderer:T.WebGLRenderer;camera:T.PerspectiveCamera;orbit:OrbitControls;transform:TransformControls;content:T.Group;meshes:Map<string,T.Object3D>;outline:T.BoxHelper|null;drag:{id:string;base:number}|null;water:T.Mesh|null;frame:()=>void}|null>(null);
  useEffect(()=>{
    ...
    const state={scene,renderer,camera,orbit,transform,content,meshes:new Map<string,T.Object3D>(),outline:null as T.BoxHelper|null,drag:null as {id:string;base:number}|null,water:null as T.Mesh|null,frame};runtime.current=state;
    ...
    const reduced=window.matchMedia("(prefers-reduced-motion: reduce)").matches;let animation=0;
    const draw=(time:number)=>{animation=requestAnimationFrame(draw);orbit.update();constrain();state.outline?.update();if(state.water&&!reduced){const pos=state.water.geometry.getAttribute("position");for(let i=0;i<pos.count;i++){const x=pos.getX(i),z=pos.getY(i);pos.setZ(i,Math.sin(x*43+time*.0009)*Math.cos(z*31+time*.0007)*.001);}state.water.geometry.computeVertexNormals();pos.needsUpdate=true;}renderer.render(scene,camera);};animation=requestAnimationFrame(draw);setMessage("");
  ...
  useEffect(()=>{
    ...
    disposeObject(w.content);w.content.clear();w.meshes.clear();w.water=null;

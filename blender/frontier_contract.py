...
def accept_proposal(proposal, *, previous, base_revision, observations, requests=(), protected_ids=()):
    ...
    for item in resolutions:
        ...
        if item["status"] == "resolved_changed":
            before = old.get(target["objectId"])
            after = next((obj for obj in effective["objects"] if obj["id"] == target["objectId"]), None)
            geometry_fields = ("asset", "position_m", "size_m", "yaw_deg", "seed")
            if before is not None and after is not None and not any(before[field] != after[field] for field in geometry_fields):
                raise FrontierError("Resolved change needs an actual same-object geometry change; a label edit is insufficient")
